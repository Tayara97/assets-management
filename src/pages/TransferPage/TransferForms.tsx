...
const TransferForms: React.FC<TransferFormsProps> = ({ selectedAsset, setShowAllForms, getAllAssets }) => {
  ...
  const handleUserTransfer = (values: any) => {
    console.log(values);
  };
  ...
  return (
    <>
      ...
      <div className="flex justify-center gap-8 mt-16">
        ...
        <Button onClick={() => setShowForm("location")}>Transfer Location</Button>
        <Button onClick={() => setShowForm("both")}>Transfer Both</Button>
      </div>
      <div className="form w-96 m-auto mt-10 text-center">
        {showForm === "location" && (
          <TransferLocationsForm selectedAsset={selectedAsset} onFinish={handleLocation} />
        )}
        {showForm === "user" && (
          <TransferUsersForm selectedAsset={selectedAsset} onFinish={handleUserTransfer} />
        )}
